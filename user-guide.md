...
- 從 Figma 截取各狀態畫面
- 產出**完整測試清單**，每個功能區塊都包含四種測試類型：

| 類型 | 代號 | 在測什麼 |
|------|------|---------|
| 📐 UI 測試 | UI- | 文案、顏色、排版、各種視覺狀態 |
| ⚙️ 功能測試 | F- | 按鈕行為、流程、資料顯示 |
| 🔲 邊界測試 | B- | 0值、最大值、時間邊界、空狀態 |
| 🚫 負面測試 | N- | 未登入、重複操作、錯誤處理 |
...
測試完成後，Claude 會在 Notion QA 文件下**建立子頁面**，用表格呈現所有測試結果。

格式長這樣：

```
🎯 [QA Hunter] 跨週簽到 驗收回報 2026-06-02

測試日期：2026-06-02｜環境：UAT｜PASS：9／FAIL：1／待確認：0／未測：2

## 📐 UI 測試
| # | 測試項目 | 預期結果 | 實際結果 | 結果 |
|---|---------|---------|---------|------|
| A-1 | 標題文案 | 「跨週獎勵」 | 「跨週獎勵」 | ✅ |
| A-2 | 按鈕 disabled 顏色 | 灰色 | 灰色 | ✅ |

## ⚙️ 功能測試
| # | 測試項目 | 前置條件 | 預期結果 | 實際結果 | 結果 |
|---|---------|---------|---------|---------|------|
| B-1 | 點擊「領取」按鈕 | 達標 + 未領 | 跳出領取彈窗 | 正常 | ✅ |
| B-2 | 領取後按鈕狀態 | 已領取 | 按鈕消失 | 正常 | ✅ |
...
| # | 問題描述 | 嚴重度 |
|---|---------|--------|
| C-3 | 連線失敗 banner 未自動消失 | 中 |
```

**重點**：所有測試項目都會出現，即使是「未測」也會列出並說明原因，不會漏項。
